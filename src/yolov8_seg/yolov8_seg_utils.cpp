...
// LetterBox(temp_img, borderImg, temp_param, input_size, false, false, true, 32);  Vec4d temp_param = { 1,1,0,0 }; // 目前这个参数的作用未知
    ...
    bool autoShape, bool scaleFill, bool scaleUp, int stride, const cv::Scalar& color)
{
    ...
    float r = std::min((float)newShape.height / (float)shape.height,  // 640 / 1280  ; 640 / 800 
        (float)newShape.width / (float)shape.width);  //取最小值也就是首先按照长边对齐
    if (!scaleUp)
        r = std::min(r, 1.0f);
 
    float ratio[2]{ r, r };
    int new_un_pad[2] = { (int)std::round((float)shape.width * r),(int)std::round((float)shape.height * r) }; 
    // 对齐完之后 长边1280->640 短边 800->400
    auto dw = (float)(newShape.width - new_un_pad[0]); // dw表示宽度上需要补齐的像素长度
    auto dh = (float)(newShape.height - new_un_pad[1]); // dh表示高度上需要补齐的像素长度
    ...
    params[1] = ratio[1];
    params[2] = left;
    params[3] = top; // 为图像添加边框 形状变为640 640 之后进行输出
...
void GetMask2(const Mat& maskProposals, const Mat& mask_protos, OutputSeg& output, const MaskParams& maskParams) {
    ...
    //crop from mask_protos  temp_rect表示的是目标检测的矩形框 // 意思是将矩形框的空间缩放到掩码的空间中
...
void DrawPred(Mat& img, vector<OutputSeg> result, std::vector<std::string> classNames, vector<Scalar> color) {
    ...
    for (int i = 0; i < result.size(); i++) {
        ...
        top = result[i].box.y;
        int color_num = i;
        rectangle(img, result[i].box, color[result[i].id], 2, 8); // 最后两个参数表示线的厚度和类型
