...
├── matplotplusplus/  # 已包含：Plotvf/Plotbc 绘图（需系统安装 gnuplot）
└── spdlog/           # 已包含：日志输出
```
...
| `BUILD_EXAMPLES` | `ON` | 是否编译示例程序 `bubbleID_example` |
...
int main() {
    ...
    std::string savefolder = "./result";       // 结果保存文件夹（下含 data/、detection_vis/、figures/）
    ...
    // 执行气泡检测和追踪（置信度 0.5；true 表示保存每帧检测可视化到 savefolder/detection_vis/）
    analysis.Generate(0.5, true);

    // 绘制并保存图表（仅写文件，不弹 GUI）
    analysis.Plotvf();  // 蒸汽分数图 -> figures/vaporfig_*.png
    analysis.Plotbc();  // 气泡数量图 -> figures/bcfig_*.png
    analysis.PlotInterfaceVelocity(0);  // 界面速度图 -> figures/velocity_*_0.png
...
├── weights/                # ONNX 模型
│   └── bubble_seg.onnx
├── result/                 # 运行输出目录（仅 .gitkeep 入库，生成文件由 .gitignore 忽略）
│   ├── .gitkeep
│   ├── data/               # txt 数据
│   ├── detection_vis/      # 检测可视化 jpg
│   └── figures/            # 分析图表 png
│
...
5. **绘图依赖**: 使用 `Plotvf()` 和 `Plotbc()` 需系统安装 **gnuplot**（Matplot++ 后端）：`sudo apt-get install gnuplot`。绘图以 quiet 模式运行，仅保存到文件，不弹 GUI；若仍出现窗口，可设置 `export GNUTERM=dumb` 再运行。

6. **日志**: 输出使用 **spdlog**，可通过 spdlog 级别或 sink 配置控制。

7. **性能**: 大型视频处理时间较长，建议使用 GPU 加速。
